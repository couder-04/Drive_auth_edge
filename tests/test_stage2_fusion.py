...
@pytest.mark.skipif(
    not (STORE / "trust_fusion.onnx").exists() or not _has_mod("onnxruntime"),
    reason="trust_fusion.onnx not trained or onnxruntime ([onnx] extra) missing",
)
...
@pytest.mark.skipif(
    not _has_mod("cv2"),
    reason="OpenCV ([face] extra) missing — PAD features need cv2",
)
